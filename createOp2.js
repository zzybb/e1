        ...
        for (let i = j; i <= oldEnd; i++) {
            if (isUse < newStrNum) {
                ...
                if (newIndex) {
                    ...
                    let deletePos = getCharPos(copyStr,oldStrList[i].id)
        ...
        if (move) {
            ...
            for (let i = newStrNum - 1; i >= 0; i--) {
                if (source[i] === -1) {
                    ...
                    let insertPos = getCharPos(copyStr,newStrList[i + newStart + 1] ? newStrList[i + newStart + 1].id : null);
                    insertPos = insertPos === -1 ? copyStr.length : insertPos;
                    ...
                    const oldPos = getCharPos(copyStr,newStrList[i + newStart].id);
                    deleteStrFunc(copyStr,newStrList[i + newStart],oldPos)
                    let newPos = getCharPos(copyStr,newStrList[i + newStart + 1] ? newStrList[i + newStart + 1].id : null);
                    newPos = newPos === -1 ? copyStr.length : newPos;
                    op.push(new Operation({
                        type: 'move',
                        string: moveStr,
                        p: [oldPos, newPos],
                    ...
                    insertStrFunc(copyStr,newStrList[i + newStart],newPos)
