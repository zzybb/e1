...
const fs = require('fs')
const opList = require('../data/nullTo/op_4000.json')
...
function createOp(opNum) {
    ...
    console.log(curStr)
    let str = JSON.stringify(opList, null, "\t")
    fs.writeFile('op_1000.json', str, function (err) {
        if (err) {
            console.log(err)
        }
    })
